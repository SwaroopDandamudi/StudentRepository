""" Program which has function which returns 3 days after Feb 27 in 2019, 2020
    ...
    delimiter
    Program contains a class which reads all the files with .py extension in
...
def file_reader(path: str, fields: int, sep: str = ',', header: bool = False)\
    ...
    # Raise filenotfound error if the file doesnt exist
    else:
        a = list()
        with fp:
            if header:
            ...
            for linenumber, line in enumerate(fp):
                ...
                if len(b) != fields:
                    raise ValueError(
                        f"{path[(path.rfind('/'))+1:]} has {len(b)} fields on\
                        line {linenumber+1} but expected {fields} ")
...
class FileAnalyzer:
    """ class which returns the number of classes, functions, characters, lines
        in a python file in a directory and make a pretty table with the
        details
    """
            ...
            for file in files:
                if file.endswith(".py"):
                    try:
                        ...
                        raise FileNotFoundError("Specified file cannot be\
                                                opened for reading")
                    # Raise filenotfound error if file doent have permissions
                    else:
                        with fp:
                            classes_num, functions_num, lines_num,
                            characters_num = 0, 0, 0, 0
                            filename_0 = file
                            for line in fp:
                                line = line.strip()
                                lines_num += 1
    ...
    def pretty_print(self) -> None:
        """ This method tabulates the pretty table with the values
            from dictionary """
        pretty_table = PrettyTable(field_names=["File Name", "Classes",
                                                "Functions", "Lines",
                                                "Characters"])
...
if __name__ == "__main__":
    print(file_reader(
        "C:/Users/shant/Documents/GitHub/StudentRepository/students.txt", 3,
        ';', True))
    print(file_reader(
        "C:/Users/shant/Documents/GitHub/StudentRepository/instructors.txt", 3,
        '|', True))
    print(file_reader(
        "C:/Users/shant/Documents/GitHub/StudentRepository/grades.txt", 4,
        '|', True))
    print(file_reader(
        "C:/Users/shant/Documents/GitHub/StudentRepository/majors.txt", 3,
        '\t', True))
