...
class University:
    ...
    def student_grades_table_db(self, db_path) -> None:
        """ This method connects to the specified database and returns
            the rows from the tables and populates Student Grade Summary
            pretty table """
        db: sqlite3.Connection = sqlite3.connect(db_path)
        pretty_table_student_summary = PrettyTable(field_names=["Name",
                                                                "CWID",
                                                                "Course",
                                                                "Grade",
                                                                "Instructor"])
        pretty_table_student_summary.title = "Student Grade Summary"
        ...
        for row in db.execute("select s.Name as Name, s.CWID as CWID,\
            ...
            pretty_table_student_summary.add_row(row)
        db.close()
        print(pretty_table_student_summary)
